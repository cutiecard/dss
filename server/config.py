...
def parse_model_name(name: str) -> tuple[str, bool]:
    """Parse a public model id and optional suffixes.

    Returns (base_name, thinking) where `base_name` is the key present in
    `MODEL_MAP` and `thinking` is True if the model name included the
    ":thinking" variant.
    """
    parts = name.split(":")
    base = parts[0]
    variants = parts[1:]
    thinking = "thinking" in variants
