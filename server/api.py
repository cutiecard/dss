...
async def chat_completions(req: ChatCompletionRequest):
    ...
    # Parse model name variants (e.g. "deepseek-chat:thinking"). A thread's
    # model is fixed when it's created, so on resume we ignore `model` and let
    # the existing thread's model stand.
    base_model, model_thinking = parse_model_name(req.model)
    model_type = None if req.conversation_id else resolve_model_type(base_model)
    # `thinking` can be supplied either as a top-level flag or encoded in the
    # model name; the model name variant takes precedence.
    thinking_flag = model_thinking or req.thinking
    ...
    if req.stream:
        def gen():
            stream = client.stream(
                ...
                model=model_type, thinking=thinking_flag, search=req.search,
            )
    ...
    try:
        reply = await run_in_threadpool(
            ...
            model_type, thinking_flag, req.search,
        )
    except Exception as e:
        return _error(f"DeepSeek request failed: {e}")

    return completion_response(req.model, reply.text, prompt, reply.conversation_id, reasoning=reply.reasoning)
