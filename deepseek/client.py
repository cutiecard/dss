...
class StreamChunk:
    """A typed text chunk from the DeepSeek SSE stream."""
...
class DeepSeekClient:
    ...
    def chat(
        ...
        parts = list(s)
        text = "".join(p.text if isinstance(p, StreamChunk) and p.kind == "message" else p for p in parts if not (isinstance(p, StreamChunk) and p.kind == "reasoning"))
        reasoning = "".join(p.text for p in parts if isinstance(p, StreamChunk) and p.kind == "reasoning") or getattr(s, "_reasoning", None)
        return Reply(text=text, conversation_id=s.conversation_id, reasoning=reasoning)
...
class _Stream:
    ...
    def __iter__(self) -> Iterator[str]:
        ...
        # If reasoning parts were captured during parsing, join them and store
        # on the stream object so callers (e.g. `chat`) can include them.
        if meta.get("reasoning_parts"):
            self._reasoning = "".join(meta.get("reasoning_parts"))
...
def _parse_sse(lines, meta: Optional[dict] = None) -> Iterator[StreamChunk]:
    """Parse DeepSeek's SSE stream into typed chunks (message vs reasoning)."""
    fragments: list[dict] = []               # full fragment objects
    active_fragment_index: Optional[int] = None
    active_field: Optional[str] = None

    def emit_fragment_content(index: int, content: str):
        """Emit content of a fragment, classifying by its type."""
        if not content:
            return
        ftype = fragments[index].get("type") if index is not None and index < len(fragments) else None
        if ftype in ("THINK", "REASONING", "THINKING"):
            if meta is not None:
                meta.setdefault("reasoning_parts", []).append(content)
            yield StreamChunk("reasoning", content)
        else:
            yield StreamChunk("message", content)
    ...
    for line in lines:
        ...
        if isinstance(v, dict) and "response" in v:
            ...
            new_fragments = v["response"].get("fragments", [])
            fragments.clear()
            fragments.extend(new_fragments)

            # Emit initial content from each fragment
            for i, frag in enumerate(fragments):
                content = frag.get("content")
                if content:
                    yield from emit_fragment_content(i, content)

            # Active target defaults to the last fragment's content
            if fragments:
                active_fragment_index = len(fragments) - 1
                active_field = "content"
            continue

        # Special: APPEND to the fragments list itself
        if obj.get("p") == "response/fragments" and obj.get("o") == "APPEND" and isinstance(v, list):
            start_index = len(fragments)
            fragments.extend(v)

            # Emit any content that came with the new fragment(s)
            for i, frag in enumerate(v):
                idx = start_index + i
                content = frag.get("content")
                if content:
                    yield from emit_fragment_content(idx, content)

            # New fragment becomes the active target for subsequent appends
            if fragments:
                active_fragment_index = len(fragments) - 1
                active_field = "content"
            continue

        # Path‑setting frame (e.g. response/fragments/0/content or .../-1/elapsed_secs)
        if "p" in obj:
            path = obj["p"]
            parts = path.split("/")
            ...
            try:
                if "fragments" in parts:
                    idx_pos = parts.index("fragments")
                    if idx_pos + 1 < len(parts):
                        idx_str = parts[idx_pos + 1]
                        if idx_str == "-1":
                            active_fragment_index = len(fragments) - 1 if fragments else None
                        else:
                            active_fragment_index = int(idx_str)
                        active_field = parts[idx_pos + 2] if idx_pos + 2 < len(parts) else None
            except (ValueError, IndexError):
                pass

            # Capture message_id
            if meta is not None and path.endswith("message_id") and isinstance(v, int):
                meta["message_id"] = v

            # APPEND or implicit append (no 'o') with string
            if isinstance(v, str) and (obj.get("o") in (None, "APPEND")):
                if active_field == "content":
                    if active_fragment_index is not None and active_fragment_index < len(fragments):
                        yield from emit_fragment_content(active_fragment_index, v)
                    else:
                        yield StreamChunk("message", v)  # safe default
                elif active_field in ("reasoning", "thinking", "think"):
                    if meta is not None:
                        meta.setdefault("reasoning_parts", []).append(v)
                    yield StreamChunk("reasoning", v)

            # SET: update fragment field (e.g. type)
            elif obj.get("o") == "SET" and isinstance(v, str):
                if active_field == "type" and active_fragment_index is not None and active_fragment_index < len(fragments):
                    fragments[active_fragment_index]["type"] = v
            continue

        # Bare append (no path) uses current active target
        if isinstance(v, str):
            if active_field == "content":
                if active_fragment_index is not None and active_fragment_index < len(fragments):
                    yield from emit_fragment_content(active_fragment_index, v)
                else:
                    yield StreamChunk("message", v)
            elif active_field in ("reasoning", "thinking", "think"):
                if meta is not None:
                    meta.setdefault("reasoning_parts", []).append(v)
                yield StreamChunk("reasoning", v)
